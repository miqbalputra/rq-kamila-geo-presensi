...
if ($method === 'GET' && isset($_GET['check'])) {
    try {
        ...
        $phw = (int)date('w', strtotime($tanggal));
        $isWeekend = ($phw == 0 || $phw == 6);
        
        $isWorkday = !$holiday && !$isWeekend;
        $offDayMessage = null;
        $offDayType = null;

        // Custom Schedule Check for specific User
        $user_id = $_GET['user_id'] ?? null;
        if ($user_id) {
            $stmt_user = $pdo->prepare("SELECT active_days FROM users WHERE id = ?");
            $stmt_user->execute([$user_id]);
            $userData = $stmt_user->fetch();
            
            if ($userData && !empty($userData['active_days'])) {
                $activeDays = explode(',', $userData['active_days']);
                // Convert PHP [0=Sun...6=Sat] to [1=Mon...7=Sun]
                $targetDay = ($phw == 0) ? "7" : (string)$phw;
                
                if (!in_array($targetDay, $activeDays)) {
                    $isWorkday = false;
                    $offDayType = 'off_schedule';
                    $offDayMessage = 'Bukan hari aktif bekerja Anda';
                }
            }
        }
        ...
        $response = [
            ...
            'isWeekend' => $isWeekend,
            'isWorkday' => $isWorkday,
            'holidayName' => $holiday ? $holiday['nama'] : ($offDayMessage ?: null),
            'holidayType' => $holiday ? $holiday['jenis'] : ($offDayType ?: null),
            'dayName' => ['Minggu', 'Senin', 'Selasa', 'Rabu', 'Kamis', 'Jumat', 'Sabtu'][$phw]
        ];
