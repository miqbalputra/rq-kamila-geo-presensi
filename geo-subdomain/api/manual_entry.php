...
if ($method === 'POST') {
    ...
    try {
        ...
        // Insert presensi manual
        $stmt = $pdo->prepare("
            INSERT INTO attendance_logs 
            (user_id, nama, tanggal, status, jam_masuk, jam_pulang, jam_hadir, jam_izin, jam_sakit, keterangan, metode, manual_reason, created_by)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'manual', ?, ?)
        ");

        $keterangan = "Manual Entry: " . $data['reason'];
